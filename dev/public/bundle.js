...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.parseExpr = function () {
        switch (this.peek().type) {
            ...
            case tokenUtil_1.TokenType.IDENTIFIER:
    ...
    Parser.prototype.parseIdentifier = function () {
        var tok = this.expect(tokenUtil_1.TokenType.IDENTIFIER);
        var keys = [tok.value];
        while (this.accept(tokenUtil_1.TokenType.OPERATOR)) {
            var t = this.expect(tokenUtil_1.TokenType.IDENTIFIER);
            keys.push(t.value);
        }
        return new LNode(NodeType.DATA, { keys: keys });
    };
    return Parser;
}());
;
...
function renderNode(node) {
    ...
    switch (node.type) {
        case parser_1.NodeType.DATA:
            services_1.fetch(opts.keys).subscribe(function (data) { div.innerText = data.toString(); }, function (err) { console.log(err); });
            break;
...
var requestStack = [];
var database = [];
var dataObs = new rxjs_1.Observable(function (observer) {
...
function fetch(keys, filters) {
    if (socket.readyState === 1) {
        socket.send(JSON.stringify({ keys: keys, filters: filters }));
